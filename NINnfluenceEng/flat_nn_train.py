...
def main(_):
    ...
    #def model(data_ego):    
        ...
        data_ego = tf.transpose(data_ego)
        input_layer = tf.concat(1, [data_env,data_ego])
        input_layer = tf.reshape(input_layer, [27, 1])
        
        with tf.variable_scope('hidden1') as scope:
            #weight_layer_1 = tf.Variable(tf.random_gamma(shape = [27,20], alpha = 1.0, beta = 0.5))
            weight_layer_1_env = tf.Variable(tf.random_normal(shape = [20,20] , stddev=1.0 , mean=0.0))
            weight_layer_1_ego = tf.Variable(tf.random_normal(shape = [7,20] , stddev=4.0 , mean=0.0))
            weight_layer_1 = tf.concat(0 , [weight_layer_1_env,weight_layer_1_ego])
            variable_summaries(weight_layer_1)
            biases_layer_1 = tf.Variable(tf.fill([20], 0.1))
        ...
        with tf.variable_scope('hidden3') as scope:
            ...
            hidden_layer_5 = tf.nn.tanh(tf.matmul(hidden_layer, weight_layer_5) + biases_layer_5, name=scope.name)
    ...
    with tf.Session() as sess:
        ...
        for loop in range(1):
            sum_loss = []
            env_data_batch, ego_data_batch,target_data_batch = env_summary.get_env(False)
            for step in range(env_data_batch.shape[0]):
                feed_dict = {data_env_node: env_data_batch[step],
                       data_ego_node: ego_data_batch[step],
                       target_node : target_data_batch[step]}
                if step % 1000 == 0:
                ...
                #print('Step :', loop,'-',step , 'time_lapse' , 1000 * elapsed_time )
                    ...
                    print('input was :',ego_data_batch[step])
                    print('predicted :', output,' ; actual:',target_data_batch[step])
                    print('--')
                #print('x_a,y_a (predicted ):',output,' | (actual)', target_data_batch[step])
                #print('--')
            print('loss statistics after loop ',loop, ':', 'mean=',np.mean(sum_loss),' median=',np.median(sum_loss),' max=',np.max(sum_loss),' min=',np.min(sum_loss))
        eval_loss = []
        ...
        for step in range(env_eval_data_batch.shape[0]):
            ...
            #print('x_a,y_a (predicted ):',output,' | (actual)', target_data_batch[step])
            #print('--')
        res_str = 'loss statistics after flat-nn run :', 'mean=',np.mean(eval_loss),' median=',np.median(eval_loss),' max=',np.max(eval_loss),' min=',np.min(eval_loss) 
        print(res_str)
        return res_str
